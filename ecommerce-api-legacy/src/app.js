...
async function createApp() {
    ...
    // Auth desligada por padrão (limitação aceita — AP-06): ver middlewares/auth.js.
    const authMiddleware = config.auth.enforce ? createAuthMiddleware(tokenService) : undefined;
...
async function start() {
    ...
    app.listen(config.port, () => {
        logger.info('server.started', { port: config.port, authEnforced: config.auth.enforce });
    });
}
